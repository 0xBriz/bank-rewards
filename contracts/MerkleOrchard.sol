...
contract MerkleOrchard {
    ...
    function getRemainingBalance(IERC20 token, address distributor)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getNextDistributionId(IERC20 token, address distributor)
        external
        view
        returns (uint256)
    {
    ...
    function isClaimed(
        ...
        (
            uint256 distributionWordIndex,
            uint256 distributionBitIndex
        ) = _getIndices(distributionId);
    ...
    function claimDistributionsWithCallback(
        ...
        require(msg.sender == claimer, "user must claim own balance");
        _processClaims(claimer, address(callbackContract), claims, tokens);
    ...
    function createDistribution(
        ...
        bytes32 channelId = _getChannelId(token, distributor);
        require(
            _nextDistributionId[channelId] == distributionId ||
                _nextDistributionId[channelId] == 0,
    ...
    function _getChannelId(IERC20 token, address distributor)
        private
        pure
        returns (bytes32)
    {
    ...
    function _processClaims(
        ...
        address recipient,
        Claim[] memory claims,
        IERC20[] memory tokens // bool asInternalBalance
    ) internal {
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            ...
            // New scope to avoid stack-too-deep issues
            {
                (
                    uint256 distributionWordIndex,
                    uint256 distributionBitIndex
                ) = _getIndices(claim.distributionId);

                if (
                    currentChannelId ==
                    _getChannelId(tokens[claim.tokenIndex], claim.distributor)
                ) {
                    ...
                    currentChannelId = _getChannelId(
    ...
    function _setClaimedBits(
        ...
        _claimedBitmap[channelId][claimer][wordIndex] =
            currentBitmap |
            newClaimsBitmap;
    }
     ...
     */
    function _deductClaimedBalance(
        bytes32 channelId,
        uint256 balanceBeingClaimed
    ) private {
        require(
